...
export default function CheckoutPage() {
  const router = useRouter(); const { items, subtotal } = useCart(); const { data: customer, isLoading: authLoading } = useCustomer(); const [loading, setLoading] = useState(false); const [error, setError] = useState("");
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (response.status === 401) return router.push("/login?next=/checkout"); if (!response.ok || !result?.paymentUrl) return setError(result?.message || "ثبت سفارش ناموفق بود."); router.push(result.paymentUrl);
  }
  if (authLoading) return <main className="min-h-[60vh] p-16 text-center">در حال بررسی حساب کاربری...</main>;
  if (!customer) return <main className="mx-auto min-h-[60vh] max-w-xl px-4 py-16 text-center"><h1 className="text-2xl font-black">برای ادامه خرید وارد شوید</h1><p className="mt-3 text-slate-500">سبد خرید شما حفظ می‌شود و پس از ورود به همین مرحله برمی‌گردید.</p><div className="mt-7 flex justify-center gap-3"><Link href="/login?next=/checkout" className="rounded-xl bg-red-600 px-6 py-3 font-bold text-white">ورود</Link><Link href="/register?next=/checkout" className="rounded-xl border px-6 py-3 font-bold">ثبت‌نام</Link></div></main>;
  ...
  return <main className="mx-auto max-w-6xl px-4 py-10"><h1 className="mb-8 text-3xl font-black">تسویه‌حساب</h1><form onSubmit={submit} className="grid gap-6 lg:grid-cols-[1fr_360px]">
    <section className="grid gap-4 rounded-3xl border bg-white p-5 sm:grid-cols-2">{fields.map(([name, label]) => <label key={name} className={name === "address" || name === "note" ? "sm:col-span-2" : ""}><span className="mb-2 block text-sm font-bold">{label}</span>{name === "address" || name === "note" ? <textarea name={name} required={name === "address"} rows={name === "address" ? 3 : 2} className="w-full rounded-xl border px-4 py-3 outline-none focus:border-red-500"/> : <input name={name} defaultValue={name === "fullName" ? customer.fullName : name === "mobile" ? customer.mobile : name === "email" ? customer.email : ""} required={name !== "email"} inputMode={name === "mobile" || name === "postalCode" ? "numeric" : undefined} className="w-full rounded-xl border px-4 py-3 outline-none focus:border-red-500"/>}</label>)}</section>
