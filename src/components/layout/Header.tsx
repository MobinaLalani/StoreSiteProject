...
export default function Header() {
  ...
  return <motion.header initial={{ opacity: 0, y: -25 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="relative z-40 border-b bg-white">
    <Container><div className="flex min-h-20 flex-wrap items-center justify-between gap-3 py-3 md:flex-nowrap md:py-0">
      <Link href="/home" className="max-w-[55vw] truncate text-xl font-black text-red-500 sm:text-2xl md:max-w-none md:text-3xl">{data?.store.name || "فروشگاه"}</Link>
      <ProductSearch />
