...
export async function POST(request: NextRequest) {
  try {
    ...
    const identity = parsed.data.identity.includes("@")
      ? normalizeEmail(parsed.data.identity)
      : normalizeMobile(parsed.data.identity);
    ...
    if (
      ...
      return NextResponse.json(
        {
          message: "موبایل/ایمیل یا رمز عبور اشتباه است.",
        },
    ...
    setCustomerCookie(response, request, token);
